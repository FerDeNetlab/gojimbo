...
export default function AdminGymsPage() {
    ...
    return (
        <div>
            <div className="mb-6">
                <h1 className="text-2xl font-black text-jimbo-white">Gimnasios registrados</h1>
                <p className="text-jimbo-muted text-sm mt-1">{clients.length} clientes en la plataforma</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <div className="bg-jimbo-card border border-jimbo-border rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-xl font-black text-jimbo-white">{clients.length}</p>
                        <p className="text-xs text-jimbo-muted">Total gyms</p>
                    </div>
                </div>
                <div className="bg-jimbo-card border border-jimbo-border rounded-xl p-4 flex items-center gap-3">
                    <CreditCard className="w-8 h-8 text-jimbo-accent" />
                    <div>
                        <p className="text-xl font-black text-jimbo-white">{clients.filter(c => c.subscriptionStatus === "active").length}</p>
                        <p className="text-xs text-jimbo-muted">Suscripciones activas</p>
                    </div>
                </div>
                <div className="bg-jimbo-card border border-jimbo-border rounded-xl p-4 flex items-center gap-3">
                    <Users className="w-8 h-8 text-jimbo-accent" />
                    <div>
                        <p className="text-xl font-black text-jimbo-white">{clients.filter(c => c.hasJimboKit).length}</p>
                        <p className="text-xs text-jimbo-muted">Con JimboKit</p>
                    </div>
                </div>
            </div>
            ...
            <div className="relative mb-6">
                ...
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre o email del dueño..."
            ...
            {loading ? (
                ...
                <div className="bg-jimbo-card border border-jimbo-border rounded-2xl p-12 text-center">
                    ...
                    <p className="text-jimbo-muted text-sm">No hay gimnasios registrados.</p>
                </div>
            ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-jimbo-border text-jimbo-muted text-left">
                                    ...
                                    <th className="px-5 py-3 font-medium hidden lg:table-cell">Miembros aprox.</th>
                            ...
                            <tbody>
                                {filtered.map((c) => (
                                    <tr key={c.id} className="border-b border-jimbo-border/50 hover:bg-jimbo-dark/50 transition-colors">
                                        <td className="px-5 py-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-jimbo-muted text-xs flex items-center gap-1">/{c.slug} <ExternalLink className="w-3 h-3" /></p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-5 py-3 hidden lg:table-cell text-jimbo-muted">{c.approxMembers || "—"}</td>
                                        <td className="px-5 py-3">
                                            <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${statusBadge(c.subscriptionStatus)}`}>
                                        ...
                                        <td className="px-5 py-3 text-right text-jimbo-muted text-xs">
