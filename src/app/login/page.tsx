...
async function getRedirectUrl(): Promise<string> {
    try {
        ...
        if (data.gym?.slug) return `/${data.gym.slug}`;
        return "/onboarding";
    } catch {
        return "/onboarding";
    }
...
export default function LoginPage() {
    ...
    const handleGoogleLogin = async () => {
        // For Google, we redirect after OAuth callback
        ...
        <div className="min-h-screen bg-jimbo-black flex items-center justify-center px-6">
            ...
            <div className="relative w-full max-w-md">
                <div className="text-center mb-10">
                    ...
                    <p className="text-jimbo-muted text-sm mt-4">Inicia sesión en tu panel de gimnasio</p>
                </div>
                ...
                <div className="bg-jimbo-card border border-jimbo-border rounded-2xl p-8">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <button type="submit" disabled={loading}
                            className="w-full bg-jimbo-white text-jimbo-black py-3 rounded-xl font-bold text-base hover:bg-jimbo-accent transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer">
                ...
                <p className="text-center text-jimbo-muted text-sm mt-6">
                    ¿No tienes cuenta?{" "}
                    <Link href="/register?plan=subscription" className="text-jimbo-accent hover:text-jimbo-accent-dim transition-colors font-medium">
                        Regístrate
                    </Link>
                </p>
            </div>
